...
function calcProgression(liftId, currentWeight, allSetsCompleted, currentFailures) {
  ...
  if (allSetsCompleted) return { newWeight: currentWeight + increment, newFailures: 0, event: "increased" };
  const nf = (currentFailures || 0) + 1;
  if (nf >= 3) return { newWeight: roundN(currentWeight * 0.9, 5), newFailures: 0, event: "deload" };
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#060c16", color:"#e2e8f0", fontFamily:"'DM Sans','Segoe UI',sans-serif", maxWidth:540, margin:"0 auto", paddingBottom:100 }}>
      <style>{CSS}</style>

      <header style={{ background:"linear-gradient(135deg,#0c1525,#0f1e38)", padding:"18px 20px 14px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", borderBottom:"1px solid #1e2d4a", position:"sticky", top:0, zIndex:50 }}>
      ...
      <nav style={{ display:"flex", background:"#0a1525", borderBottom:"1px solid #1e293b", position:"sticky", top:70, zIndex:49 }}>
        {[{id:"today",icon:"☀",label:"Today"},{id:"workout",icon:"🏋",label:"Workout"},{id:"progress",icon:"📈",label:"Progress"},{id:"plan",icon:"📋",label:"Plan"}].map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setWkView(null); }}
            style={{ flex:1, padding:"10px 4px", background:"none", border:"none", color: tab===t.id ? "#38bdf8" : "#475569", cursor:"pointer", fontSize:11, fontWeight:700, display:"flex", flexDirection:"column", alignItems:"center", gap:2, borderBottom: tab===t.id ? "2px solid #38bdf8" : "2px solid transparent" }}>
...
function TodayTab({ week, phase, log, sess, streak, total, updateLog, goWorkout, weights, failures, viewDate, todayStr, shiftDate }) {
  ...
  return (
    <div className="fadein">
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#0d1829", border:"1px solid #1e293b", borderRadius:12, padding:"10px 14px", marginBottom:16 }}>
        <button onClick={() => shiftDate(-1)} style={{ background:"#0a1525", border:"1px solid #1e293b", borderRadius:8, color:"#94a3b8", fontSize:22, fontWeight:700, width:36, height:36, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
          {"<"}
        </button>
        <div style={{ textAlign:"center" }}>
          <div style={{ fontWeight:700, color: isToday ? "#38bdf8" : "#e2e8f0", fontSize:16 }}>{displayDate}</div>
          {!isToday && (
            <button onClick={() => shiftDate(999)} style={{ background:"none", border:"none", color:"#475569", fontSize:11, cursor:"pointer", marginTop:2, textDecoration:"underline" }}>
              back to today
            </button>
          )}
        </div>
        <button onClick={() => shiftDate(1)} disabled={isToday} style={{ background:"#0a1525", border:"1px solid #1e293b", borderRadius:8, color: isToday ? "#1e293b" : "#94a3b8", fontSize:22, fontWeight:700, width:36, height:36, cursor: isToday ? "default" : "pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
          {">"}
        </button>
      </div>
              ...
              <div key={id} style={{ background:"#0a1525", borderRadius:8, padding:"8px 10px", border:"1px solid #1e293b" }}>
                ...
                {f > 0 && (
                  <div style={{ fontSize:11, marginTop:2, color:f>=2?"#f87171":"#f59e0b" }}>
                    {f>=2 ? "1 miss from deload" : f + "/3 misses"}
                  </div>
                )}
              </div>
            );
      ...
      <Card>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <input type="number" placeholder="Enter steps" value={steps||""} onChange={e => updateLog({ steps: parseInt(e.target.value)||0 })}
            style={{ flex:1, background:"#0a1525", border:"1px solid #1e3a5f", borderRadius:8, color:"#e2e8f0", fontSize:22, fontWeight:700, padding:"8px 12px", outline:"none", fontFamily:"inherit" }} />
          <span style={{ fontSize:22, fontWeight:700, color:pct>=100?"#4ade80":"#cbd5e1", minWidth:52, textAlign:"right" }}>{pct}%</span>
        </div>
        <div style={{ height:6, background:"#1e293b", borderRadius:99, overflow:"hidden", marginTop:10 }}>
          <div style={{ height:"100%", borderRadius:99, width:pct+"%", background:pct>=100?"#4ade80":"#38bdf8", transition:"width 0.4s" }} />
        </div>
      ...
      <Card>
        <div style={{ fontWeight:700, color:"#e2e8f0", fontSize:15, marginBottom:10 }}>{isToday ? "Today's Workout" : "Workout"}</div>
        {sess?.completed
          ? <div style={{ color:"#4ade80", fontWeight:700 }}>Workout {sess.type} complete!</div>
          : isToday
            ? <button onClick={goWorkout} style={{ background:"linear-gradient(135deg,#1d4ed8,#0369a1)", color:"#fff", border:"none", borderRadius:10, padding:"12px 24px", fontSize:15, fontWeight:700, cursor:"pointer" }}>Log Workout</button>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {[5,6,6.5,7,7.5,8,8.5,9].map(h => (
            <button key={h} onClick={() => updateLog({ sleep: h })}
              style={{ background:log.sleep===h?"#1e3a5f":"#0a1525", border:"1px solid "+(log.sleep===h?"#38bdf8":"#1e293b"), borderRadius:8, color:log.sleep===h?"#38bdf8":"#64748b", fontSize:13, fontWeight:700, padding:"6px 10px", cursor:"pointer" }}>
              {h}h
            </button>
          ))}
        </div>
    ...
    <div className="fadein">
      {todaySess?.completed && (
        <div style={{ background:"#052e16aa", border:"1px solid #4ade8044", borderRadius:12, padding:"12px 16px", marginBottom:14 }}>
          <span style={{ color:"#4ade80", fontWeight:700 }}>Workout {todaySess.type} already logged today</span>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:14 }}>
        {["A","B"].map(t => (
          <button key={t} onClick={() => { setWkView(t); setDone(false); setSets({}); setWOver({}); setAlerts([]); }}
            style={{ flex:1, background:"#0d1829", border:"1px solid #1e293b", borderRadius:16, padding:"24px 16px", cursor:"pointer", textAlign:"center" }}>
            <div style={{ fontSize:48, fontWeight:900, color:t==="A"?"#38bdf8":"#a78bfa", lineHeight:1 }}>{t}</div>
            <div style={{ fontSize:13, color:"#94a3b8", marginTop:4 }}>{t==="A" ? "Squat / Bench / Row" : "Deadlift / OHP / Pulls"}</div>
          </button>
        ))}
      </div>
      {recent.length > 0 && (
        <div>
          ...
          {recent.map((s, i) => (
            <div key={i} style={{ background:"#0d1829", border:"1px solid #1e293b", borderRadius:10, padding:"10px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
    ...
    <div className="fadein" style={{ textAlign:"center", padding:"32px 16px" }}>
      ...
      {alerts.length > 0 && (
        <div style={{ marginBottom:20, marginTop:16 }}>
          <div style={{ fontSize:12, color:"#64748b", fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10 }}>Weight Updates</div>
          {alerts.map((a, i) => {
            const up=a.event==="increased", dl=a.event==="deload";
            return (
              <div key={i} style={{ background:dl?"#1c0a0a":up?"#052e16":"#1c1400", border:"1px solid "+(dl?"#f8717144":up?"#4ade8044":"#f59e0b44"), borderRadius:12, padding:"10px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center", textAlign:"left" }}>
                <div>
                  ...
                  <div style={{ fontSize:12, marginTop:2, color:dl?"#f87171":up?"#4ade80":"#f59e0b" }}>
                    {up ? "All sets complete — weight increased!" : dl ? "3 misses — 10% deload applied" : "Repeat this weight next session"}
                  </div>
                </div>
                <div style={{ textAlign:"right" }}>
      ...
      <button onClick={() => setWkView(null)} style={{ background:"linear-gradient(135deg,#1d4ed8,#0369a1)", color:"#fff", border:"none", borderRadius:10, padding:"12px 28px", fontSize:15, fontWeight:700, cursor:"pointer" }}>
        Back
      </button>
    </div>
  );
  ...
  return (
    <div className="fadein">
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        <button onClick={() => setWkView(null)} style={{ background:"none", border:"none", color:"#94a3b8", cursor:"pointer", fontSize:22, padding:4 }}>{"<"}</button>
      ...
      {sections.map(sec => {
        ...
        return (
          <div key={sec.label} style={{ marginBottom:22 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#64748b", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:8 }}>{sec.label}</div>
            {list.map(ex => {
              const n  = getSets(ex.scheme);
              const ad = ex.type!=="mob" && allDone(ex.id, n);
              const f  = failures[ex.id] || 0;
              return (
                <div key={ex.id} style={{ background:"#0d1829", border:"1px solid "+(ad?"#4ade8033":"#1e293b"), borderRadius:12, padding:"12px 14px", marginBottom:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                    <div style={{ flex:1 }}>
                      ...
                      {ex.isMain && f > 0 && (
                        <div style={{ fontSize:11, marginTop:2, color:f>=2?"#f87171":"#f59e0b" }}>
                          {f>=2 ? f+"/3 — next miss = deload" : f+"/3 misses"}
                        </div>
                      )}
                    </div>
                    {ex.isMain && (
                      <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                        ...
                        <input type="number" value={wOver[ex.id] !== undefined ? wOver[ex.id] : weights[ex.id]} placeholder={""+weights[ex.id]}
                          onChange={e => setWOver(p => ({ ...p, [ex.id]: e.target.value }))}
                          style={{ width:60, background:"#0a1525", border:"1px solid #1e3a5f", borderRadius:8, color:"#38bdf8", fontSize:16, fontWeight:700, padding:"5px 6px", outline:"none", fontFamily:"inherit", textAlign:"center" }} />
                      </div>
                    )}
                  </div>
                  {ex.type !== "mob" ? (
                    <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
                      {Array.from({length:n}).map((_, i) => {
                        const isDone = sets[ex.id+"_"+i];
                        return (
                          <button key={i} onClick={() => toggle(ex.id, i)}
                            style={{ background:isDone?"#052e16":"#0a1525", border:"1px solid "+(isDone?"#4ade80":"#1e3a5f"), borderRadius:8, color:isDone?"#4ade80":"#64748b", fontSize:12, fontWeight:700, padding:"6px 11px", cursor:"pointer", minWidth:52 }}>
                            {isDone ? "Done" : "Set "+(i+1)}
                          </button>
                        );
                      })}
                    ...
                    <CheckItem label="Completed" checked={!!sets[ex.id+"_0"]} onChange={v => setSets(p => ({ ...p, [ex.id+"_0"]: v }))} />
                  )}
                </div>
              );
            })}
      ...
      <button onClick={finish} style={{ background:"linear-gradient(135deg,#1d4ed8,#0369a1)", color:"#fff", border:"none", borderRadius:10, padding:"14px", fontSize:15, fontWeight:700, cursor:"pointer", width:"100%", marginTop:4 }}>
        Complete Workout
      </button>
    </div>
  );
...
function ProgressTab({ sessions, weights, logs, streak, total, failures }) {
  ...
  const liftData = (() => {
    const pts = sessions.filter(s => s.completed && s.lifts && s.lifts[sel])
  ...
  return (
    <div className="fadein">
        ...
        <div style={{ display:"flex", gap:7, flexWrap:"wrap", marginBottom:12 }}>
          {MAIN_LIFT_IDS.map(id => (
            <button key={id} onClick={() => setSel(id)}
              style={{ background:sel===id?"#1e3a5f":"#0a1525", border:"1px solid "+(sel===id?"#38bdf8":"#1e293b"), borderRadius:20, color:sel===id?"#38bdf8":"#475569", fontSize:11, fontWeight:700, padding:"3px 10px", cursor:"pointer" }}>
      ...
      <Card title="Working Weights">
        {MAIN_LIFT_IDS.map(id => {
          const f = failures[id] || 0;
          const diff = weights[id] - DEFAULT_WEIGHTS[id];
          return (
            <div key={id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:"1px solid #1e293b" }}>
              <div>
...
function PlanTab({ week, phase }) {
  ...
  const btnStyle = (id) => ({ background: sec===id ? "#1e3a5f" : "#0a1525", border: "1px solid "+(sec===id ? "#38bdf8" : "#1e293b"), borderRadius:20, color: sec===id ? "#38bdf8" : "#475569", fontSize:12, fontWeight:700, padding:"4px 12px", cursor:"pointer" });
  ...
  return (
    <div className="fadein">
      ...
      {sec==="phases" && [
        { p:1, w:"1-3",  c:"#38bdf8", t:"Re-Entry & Movement Quality",  b:["3x/week: A/B/A, B/A/B","DB Floor Press — not barbell bench","Deadlift: 1x5 top + 2x3","Focus: consistency + shoulder health","Shoulder warm-up every session"] },
        { p:2, w:"4-7",  c:"#fb923c", t:"Volume Build",                  b:["4x/week: A/B/A/B","5 lbs added when all sets complete","Reintroduce barbell bench if shoulder clears x3","Step goal: 10,500+"] },
        { p:3, w:"8-10", c:"#a78bfa", t:"Peak & Show-Ready",             b:["4x/week, max intensity","Week 10 = 40% volume deload","Keep protein high, cut sodium Week 10","Step goal: 12,000+"] },
      ].map(x => (
        <div key={x.p} style={{ background:"#0d1829", border:"1px solid #1e293b", borderLeft:"4px solid "+x.c, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
          <div style={{ display:"flex", gap:8, alignItems:"baseline", marginBottom:6 }}>
            ...
            {phase===x.p && <span style={{ background:x.c+"22", color:x.c, fontSize:10, fontWeight:700, padding:"1px 7px", borderRadius:20, border:"1px solid "+x.c+"44" }}>CURRENT</span>}
          </div>
      ...
      {sec==="nutrition" && (
        <div>
          <div style={{ background:"#1c1a0c99", border:"1px solid #f59e0b44", borderRadius:12, padding:"12px 14px", marginBottom:12 }}>
            <span style={{ color:"#f59e0b", fontWeight:700 }}>Rule: </span>
      ...
      {sec==="shoulder" && (
        <div>
          <div style={{ background:"#1c0a0a99", border:"1px solid #f8717144", borderRadius:12, padding:"12px 14px", marginBottom:12 }}>
            <span style={{ color:"#f87171", fontWeight:700 }}>Sharp pain = stop the set. </span>
          ...
          {[
            ...
            "Elbows at 45-60 degrees from torso on all pressing",
            "Narrow grip on pressing movements",
            "Face pulls and lateral raises are rehabilitative — never skip them",
          ].map((r, i) => (
            <div key={i} style={{ background:"#0d1829", border:"1px solid #1e293b", borderRadius:12, padding:"10px 14px", marginBottom:7, display:"flex", gap:10 }}>
      ...
      {sec==="milestones" && [
        ...
        <div key={i} style={{ background:"#0d1829", border:"1px solid #1e293b", borderLeft:"4px solid "+m.c, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
...
function StatCard({ label, value, unit, color }) {
  return (
    <div style={{ background:"#0d1829", border:"1px solid #1e293b", borderTop:"3px solid "+color, borderRadius:12, padding:"14px 12px", textAlign:"center" }}>
...
function Chip({ label, color }) {
  return (
    <span style={{ fontSize:11, fontWeight:700, padding:"2px 8px", borderRadius:20, background: color ? color+"22" : "#1e3a5f33", color: color || "#94a3b8", border: "1px solid "+(color ? color+"44" : "#1e3a5f") }}>
      {label}
    </span>
  );
}
...
function CheckItem({ label, checked, onChange }) {
  return (
    <div onClick={() => onChange(!checked)} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"7px 0", cursor:"pointer", borderBottom:"1px solid #0f172a" }}>
      <div style={{ width:20, height:20, borderRadius:5, border:"2px solid "+(checked?"#38bdf8":"#1e3a5f"), background: checked ? "#38bdf8" : "#0a1525", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", marginTop:1 }}>
        {checked && <span style={{ color:"#0f172a", fontSize:12, fontWeight:900 }}>✓</span>}
      </div>
      <span style={{ color:checked?"#64748b":"#cbd5e1", fontSize:13, lineHeight:1.5, textDecoration:checked?"line-through":"none", flex:1 }}>{label}</span>
    </div>
  );
}
